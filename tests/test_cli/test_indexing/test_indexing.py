...
def test_full_reindex(
    ...
    search_result = search_connection.search(
        body={"query": {"match_all": {}}}, index=settings.ACTIVE_SEARCH_INDEX_ALIAS
    )
    received_documents = {
        hit["_source"]["primary_field"] for hit in search_result["hits"]["hits"]
    }
